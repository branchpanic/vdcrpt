    ...
    public class Program
    {
        public static int Main(string[] args)
        {
            return Parser.Default.ParseArguments<Args>(args).MapResult(parsedArgs =>
            {
                ...
                {
                    Console.Error.WriteLine($"Output file {parsedArgs.OutputFile} already exists (use -o to overwrite)");
                    return 1;
                }
                ...
                for (var i = 0; i < parsedArgs.Iterations; i++)
                    v.ModifyBytes(Effects.Repeat(5000, 10, 20));
